...
import json
from deneb.models import download_model, load_checkpoint, model2download, str2model
from deneb.trainer import TrainerConfig, build_trainer
import yaml
from utils import *
from dataset import *
...
def deneb(dataset,args):
    yprint("Compute Deneb ...")
...
def compute_foil(args, memory, tops):
    ...
    for one_ref in [True, False]:
        ...
        dataset_name += suffix
        if args.deneb:
            deneb_acc = compute_acc(deneb, dataset, one_ref, args=args)
            collect_acc(memory, dataset_name, f"Deneb{suffix}", deneb_acc)
