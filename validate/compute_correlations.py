...
from deneb.metrics.regression_metrics import RegressionReport
from deneb.models import load_checkpoint
import argparse
from deneb.models import load_checkpoint
...
def compute_correlation_scores(memory, dataloader, pacscore, deneb, preprocess, args):
    gen = {}
    gts = {}
    ...
    _, sys_score = deneb.predict(data,cuda=True,batch_size=32)
    all_scores['Deneb'] = sys_score
    del data
...
def compute_scores(memory, pacscore, deneb, preprocess, args):
    ...
    for d in args.datasets:
        ...
        args.dataset_name = d
        memory = compute_correlation_scores(memory, dataloader, pacscore, deneb, preprocess, args)
    return memory


def compute_flickr(args,checkpoint,memory,tops):
    # Deneb
    deneb = load_checkpoint(checkpoint)
    ...
    args.device = device
    memory = compute_scores(memory, pacscore, deneb, preprocess, args)
    return memory, tops
