...
from .estimators import DenebEstimator, QualityEstimator
from .model_base import ModelBase
from .ranking import DenebRanker

str2model = {
    "DenebEstimator": DenebEstimator,
    "DenebRanker": DenebRanker,
...
def get_cache_folder():
    if "HOME" in os.environ:
        cache_directory = os.environ["HOME"] + "/.cache/torch/unbabel_deneb/"
...
def download_model(model: str, saving_directory: str = None) -> ModelBase:
    ...
    if os.path.isdir(saving_directory + model):
        ...
        raise Exception(f"{model} is not a valid Deneb model!")
...
def load_checkpoint(checkpoint: str) -> ModelBase:
    ...
    if os.path.exists(tags_csv_file):
        ...
        raise Exception(
            ...
            " Please clean your cache folder (~/.cache/torch/unbabel_deneb/) and try to download the model again."
        )
