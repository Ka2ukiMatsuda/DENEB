...
from deneb.metrics import RegressionReport
from deneb.models.model_base import ModelBase
from deneb.models.utils import average_pooling, max_pooling, move_to_cpu, move_to_cuda
...
class Estimator(ModelBase):
    ...
    def _build_loss(self):
        ...
        if self.hparams.loss == "mse":
            ...
            self.loss = nn.HuberLoss(reduction="sum", delta=0.5)
        else:
    ...
    def compute_loss(
        ...
        """
        assert torch.all((targets["score"] >= 0) & (targets["score"] <= 1)), f"gt({targets['score']}) is not in [0,1]!"
        # print("out",model_out["score"].view(-1).shape, targets["score"].shape)
    ...
    def compute_metrics(self, outputs: List[Dict[str, torch.Tensor]]) -> dict:
        ...
        # self.draw_histogram(predictions, targets)
    
        return self.metrics.compute(predictions, targets)

    def draw_histogram(self, predictions, targets):
        import matplotlib
